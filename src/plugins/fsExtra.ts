...
export async function metadata(path: string): Promise<Metadata> {
    return await invoke<BackendMetadata>('plugin:wa|metadata', { path }).then((metadata) => {
...
export async function exists(path: string): Promise<boolean> {
    return await invoke('plugin:wa|exists', { path });
}

export async function openFile(path: string) {
    return await invoke('plugin:wa|open_file', { path });
}